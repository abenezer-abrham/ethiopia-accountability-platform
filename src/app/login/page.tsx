...
import { ArrowRight, Mail, Lock, AlertCircle, CheckCircle2, Eye, EyeOff, Sparkles, ShieldCheck } from 'lucide-react';
...
function LoginForm() {
  ...
  const handleOAuthLogin = async (provider: 'google' | 'github' | 'apple') => {
    ...
    try {
      ...
      console.warn('OAuth fallback to demo mode:', err);
      router.push(next);
    } finally {
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-white">
            Welcome to Egna <span className="text-emerald-400 font-semibold text-lg font-ethiopic">እኛ</span>
          </h1>
          <p className="text-xs text-slate-400">Sign in to your accountability command center</p>
        </div>
          ...
          {magicLinkSent ? (
            <div className="p-4 bg-emerald-950/80 text-emerald-300 border border-emerald-800 rounded-xl text-center text-xs space-y-2">
              ...
              <p className="text-slate-300">
                Check your inbox at <strong className="text-white">{email}</strong> to sign in.
              </p>
              <Link href="/home">
          ...
          <div className="pt-3 border-t border-slate-800 space-y-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <button
                ...
                className="p-2 rounded-lg bg-emerald-950/60 border border-emerald-800 text-[11px] font-bold text-emerald-300 hover:bg-emerald-900/60 transition-colors text-center cursor-pointer"
              >
                🛡️ Admin
              </button>
              <button
                ...
                className="p-2 rounded-lg bg-slate-800 border border-slate-700 text-[11px] font-bold text-slate-200 hover:bg-slate-700 transition-colors text-center cursor-pointer"
              >
                👤 Member
              </button>
            </div>
